...
# Angular WebApp Deployment Script
# This script deploys the OlloLifestyle Angular webapp to production
...
pull_images() {
    ...
    # Pull the latest webapp image
    if ! docker pull dayandev/ollolifestyle-webapp:latest; then
...
cleanup_images() {
    ...
    # Keep only the 3 most recent versions of the webapp image
    local webapp_images=$(docker images --format "table {{.Repository}}\t{{.Tag}}\t{{.ID}}" | grep "ollolifestyle-webapp" | grep -v latest | tail -n +4 | awk '{print $3}')
    
    if [ ! -z "$webapp_images" ]; then
        echo "$webapp_images" | xargs -r docker rmi || true
        log_info "Old webapp images cleaned up"
    fi
}
...
verify_deployment() {
    ...
    local endpoints=("http://localhost:3000")
...
deploy() {
    log_info "Starting OlloLifestyle Angular WebApp deployment..."
    ...
    log_info "🎉 Angular WebApp deployment completed successfully!"
    echo ""
    echo "🔗 WebApp URL: http://192.168.50.98:3000"
